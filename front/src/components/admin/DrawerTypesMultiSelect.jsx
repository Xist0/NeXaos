...
/**
 * Множественный выбор «Вид и кол-во ящиков».
 * Хранит значение как строку: «84мм × 2; 116мм × 1»
 */
const parseDrawerDetail = (raw) => {
...
const DrawerTypesMultiSelect = ({
  ...
  const selected = useMemo(() => parseDrawerDetail(value), [value]);

  const toggleType = (type) => {
    const exists = selected.find((x) => x.type === type);
    if (exists) {
      const next = selected.filter((x) => x.type !== type);
      onChange(serializeDrawerDetail(next));
      return;
    }
    onChange(serializeDrawerDetail([...selected, { type, qty: 1 }]));
  };
  ...
  const addCustom = (type) => {
    ...
    onChange(serializeDrawerDetail([...selected, { type: t, qty: 1 }]));
  };
  ...
  return (
    ...
    >
      ...
      <div className={visible ? "space-y-3" : "opacity-50 pointer-events-none space-y-3"}>
        ...
        <SecureInput
          placeholder="Другой тип ящика + Enter"
          onKeyDown={(e) => {
...
export { parseDrawerDetail, serializeDrawerDetail };
export default DrawerTypesMultiSelect;
