...
const HingeSelectField = ({
  ...
  const [hingeRows, setHingeRows] = useState(() => parseHingeValue(value, items));
  const lastSyncedValue = useRef(value);

  useEffect(() => {
    if (value !== lastSyncedValue.current) {
      setHingeRows(parseHingeValue(value, items));
  ...
  const addRow = () => {
    setHingeRows((prev) => [...prev, { itemId: "", itemName: "", qty: 1 }]);
  };
  ...
  const activeRows = hingeRows.filter((r) => r.itemName);
        ...
        <div className="space-y-2">
          {hingeRows.map((row, index) => {
            ...
            return (
              <div key={index} className="flex items-center gap-2">
                ...
                >
                  <option value="">— Выберите петлю —</option>
                  {items.map((i) => (
          ...
          {activeRows.length > 0 ? (
            <div className="text-xs text-night-500 pt-1">
              Итого: {activeRows.reduce((s, r) => s + r.qty, 0)} петель
...
function parseHingeValue(text, items) {
  if (!text) return [];
...
export default HingeSelectField;
