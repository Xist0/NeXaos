...
const HangerSelectField = ({
  ...
  const [hangerRows, setHangerRows] = useState(() => parseHangerValue(value, items));
  const lastSyncedValue = useRef(value);

  useEffect(() => {
    if (value !== lastSyncedValue.current) {
      setHangerRows(parseHangerValue(value, items));
  ...
  const addRow = () => {
    setHangerRows((prev) => [...prev, { itemId: "", itemName: "", qty: 1 }]);
  };
  ...
  const selectedItem = (row) => {
    ...
    return items.find((i) => String(i.id) === String(row.itemId)) || null;
  };

  const activeRows = hangerRows.filter((r) => r.itemName);
        ...
        <div className="space-y-2">
          {hangerRows.map((row, index) => {
            ...
            return (
              <div key={index} className="flex items-center gap-2">
                ...
                >
                  <option value="">— Выберите навес —</option>
                  {items.map((i) => (
          ...
          {activeRows.length > 0 ? (
            <div className="text-xs text-night-500 pt-1">
              Итого: {activeRows.reduce((s, r) => s + r.qty, 0)} навесов
...
function parseHangerValue(text, items) {
  if (!text) return [];
...
export default HangerSelectField;
